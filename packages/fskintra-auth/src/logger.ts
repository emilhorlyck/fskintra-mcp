...
export function consoleLogger(prefix = 'fskintra-auth'): Logger {
  return {
...
/**
 * Logger that writes every level to stderr. Use this in stdio MCP servers —
 * stdout is the JSON-RPC channel and `console.info`/`debug` default to stdout
 * in Node/Bun, which would corrupt the protocol.
 */
export function stderrLogger(prefix = 'fskintra-auth'): Logger {
